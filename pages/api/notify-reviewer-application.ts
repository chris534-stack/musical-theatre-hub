...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const adminEmails = process.env.ADMIN_EMAILS; // Reverted to plural
  if (!adminEmails) { // Reverted to plural
    console.error('[NotifyReviewerApp] ADMIN_EMAILS environment variable is not set.'); // Reverted to plural
    return res.status(500).json({ error: 'ADMIN_EMAILS environment variable is not set.' }); // Reverted to plural
  }
  ...
  try {
    const mailOptions = {
      from: gmailUser,
      to: adminEmails, // Reverted to plural
      replyTo: email,
      subject: subject,
      text: textBody,
    };
